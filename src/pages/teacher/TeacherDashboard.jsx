/**
 * SAL Education - College Attendance Management System
 * Teacher Dashboard Page
 * 
 * Overview for teachers with their assigned subjects, classes, and quick actions.
 * Shows dynamic statistics based on assigned subjects.
 * 
 * VIVA NOTE: This dashboard uses the logged-in teacher's ID to fetch
 * their assigned subjects and show relevant statistics.
 */

import { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import { Card, StatsCard } from '../../components';
import { useAuth } from '../../context/AuthContext';
import { attendanceService } from '../../services/dataService';

const TeacherDashboard = () => {
  // Get current user from auth context
  const { user } = useAuth();
  
  // State for dashboard data
  const [assignedSubjects, setAssignedSubjects] = useState([]);
  const [stats, setStats] = useState({
    totalSubjects: 0,
    totalStudents: 0,
    totalClasses: 0,
    todayAttendance: 0
  });

  const loadDashboardData = async () => {
    try {
      // Get teacher's assigned subjects via teacher API
      const subjects = await attendanceService.getMySubjects();
      setAssignedSubjects(subjects);
      
      // Calculate unique classes from subject data
      const uniqueClassIds = [...new Set(subjects.map(s => s.classId))];
      
      setStats({
        totalSubjects: subjects.length,
        totalStudents: 0,
        totalClasses: uniqueClassIds.length,
        todayAttendance: 0
      });
    } catch (error) {
      console.error('Error loading dashboard data:', error);
    }
  };

  /**
   * Load teacher's data on component mount
   */
  useEffect(() => {
    loadDashboardData();
  }, []);

  return (
    <div className="space-y-6">
      {/* Welcome section */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">
            Welcome, {user?.name || 'Teacher'}!
          </h1>
          <p className="text-gray-600">Here's your teaching overview for today.</p>
        </div>
        <div className="mt-4 md:mt-0 text-sm text-gray-500">
          {new Date().toLocaleDateString('en-US', {
            weekday: 'long',
            year: 'numeric',
            month: 'long',
            day: 'numeric',
          })}
        </div>
      </div>

      {/* Stats cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatsCard
          title="Assigned Subjects"
          value={stats.totalSubjects}
          bgColor="bg-blue-500"
          icon={
            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
            </svg>
          }
        />
        <StatsCard
          title="Total Students"
          value={stats.totalStudents}
          bgColor="bg-green-500"
          icon={
            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
            </svg>
          }
        />
        <StatsCard
          title="Classes Assigned"
          value={stats.totalClasses}
          bgColor="bg-purple-500"
          icon={
            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
            </svg>
          }
        />
        <StatsCard
          title="Today's Attendance"
          value={`${stats.todayAttendance}%`}
          bgColor="bg-yellow-500"
          icon={
            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          }
        />
      </div>

      {/* Quick Actions */}
      <Card title="Quick Actions">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Link
            to="/teacher/mark-attendance"
            className="flex items-center p-4 bg-green-50 hover:bg-green-100 rounded-lg transition-colors"
          >
            <div className="p-3 bg-green-500 text-white rounded-lg mr-4">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
              </svg>
            </div>
            <div>
              <h3 className="font-semibold text-gray-800">Mark Attendance</h3>
              <p className="text-sm text-gray-600">Mark attendance for your classes</p>
            </div>
          </Link>
          
          <Link
            to="/teacher/view-attendance"
            className="flex items-center p-4 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors"
          >
            <div className="p-3 bg-blue-500 text-white rounded-lg mr-4">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
              </svg>
            </div>
            <div>
              <h3 className="font-semibold text-gray-800">View Attendance</h3>
              <p className="text-sm text-gray-600">View and analyze attendance records</p>
            </div>
          </Link>

          <Link
            to="/teacher/proxy-requests"
            className="flex items-center p-4 bg-yellow-50 hover:bg-yellow-100 rounded-lg transition-colors"
          >
            <div className="p-3 bg-yellow-500 text-white rounded-lg mr-4">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8h2a2 2 0 012 2v8a2 2 0 01-2 2H7a2 2 0 01-2-2v-2m10-8V6a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2h8a2 2 0 002-2z" />
              </svg>
            </div>
            <div>
              <h3 className="font-semibold text-gray-800">Proxy Requests</h3>
              <p className="text-sm text-gray-600">Request proxy lecture with HOD approval</p>
            </div>
          </Link>

          <Link
            to="/teacher/proxy-lectures"
            className="flex items-center p-4 bg-purple-50 hover:bg-purple-100 rounded-lg transition-colors"
          >
            <div className="p-3 bg-purple-500 text-white rounded-lg mr-4">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
            </div>
            <div>
              <h3 className="font-semibold text-gray-800">Proxy Lectures</h3>
              <p className="text-sm text-gray-600">View lectures assigned to you as proxy</p>
            </div>
          </Link>
        </div>
      </Card>

      {/* Assigned Subjects */}
      <Card title="My Assigned Subjects">
        {assignedSubjects.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {assignedSubjects.map((subject) => (
              <div
                key={subject.id}
                className="border rounded-lg p-4 hover:shadow-md transition-shadow"
              >
                <div className="flex justify-between items-start mb-2">
                  <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded text-xs font-medium">
                    {subject.code}
                  </span>
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                    subject.type === 'theory' ? 'bg-blue-100 text-blue-800' :
                    subject.type === 'lab' ? 'bg-green-100 text-green-800' :
                    'bg-purple-100 text-purple-800'
                  }`}>
                    {subject.type === 'theory+lab' ? 'T+L' : subject.type.charAt(0).toUpperCase()}
                  </span>
                </div>
                <h4 className="font-semibold text-gray-800 mb-1">{subject.name}</h4>
                <p className="text-sm text-gray-500">Class: {subject.class?.name || 'N/A'}</p>
                {subject.teachingType && (
                  <p className="text-xs text-gray-400 mt-1">Teaching: {subject.teachingType}</p>
                )}
              </div>
            ))}
          </div>
        ) : (
          <div className="text-center py-8 text-gray-500">
            <p>No subjects assigned yet.</p>
          </div>
        )}
      </Card>
    </div>
  );
};

export default TeacherDashboard;
